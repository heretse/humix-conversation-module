...
var Conversation = function Conversation(option, log, sensorModule) {


    logger = log;
    this.config = option;
    if (!this.config) this.config = require('../config.js');
...
Conversation.prototype.initMic = function () {
    ...
    var micConfig = {
        ...
        'device': self.config.stt.sttDevice || 'plughw:0'
    };
...
Conversation.prototype.say = function(input) {
    ...
    if (fs.existsSync(filename)) {
        ...
        this.sensorModule.event('pauseSpotify');
        playFile(filename);
        fsm.ready();
    } else {
